...
def func_attention(query, context, opt):
    ...
    queryT = torch.transpose(query, 1, 2)#(n_context, queryL, d)->(n_context, d, queryL)
    
    attn = torch.bmm(context, queryT)#(n_context, sourceL, d)*(n_context, d, queryL)->(n_context, sourceL, queryL)
    attn = nn.LeakyReLU(0.1)(attn)
    attn = l2norm(attn, 2)
    attn = torch.transpose(attn, 1, 2).contiguous()#(n_context, sourceL, queryL)->(n_context, queryL, sourceL)
    attn = attn.view(batch_size*queryL, sourceL)
    attn = nn.Softmax(dim=1)(attn*opt.lambda_softmax)
    #attn_filter = (attn>(1.0/sourceL)).float()
    #attn = attn*attn_filter
    attn = attn.view(batch_size, queryL, sourceL)#(n_context, queryL, sourceL)
    attnT = torch.transpose(attn, 1, 2).contiguous()#(n_context, sourceL, queryL)
    contextT = torch.transpose(context, 1, 2)#(n_context, sourceL, d)->(n_context, d, sourceL)
    weightedContext = torch.bmm(contextT, attnT)#(n_context, d, sourceL)*(n_context, sourceL, queryL)->(n_context, d, queryL)
    weightedContext = torch.transpose(weightedContext, 1, 2)#(n_context, queryL, d)
...
def func_direct_attention(query, context, caps_entity_idx_i, image_entity_idxs, opt):
    """
    query: (n_context, queryL, d)
    context: (n_context, sourceL, d)
    caps_entity_idx_i: (n_context, n_word)
    image_entity_idxs: (n_context, 36)
    """
    batch_size_q, queryL = query.size(0), query.size(1)
    batch_size, sourceL = context.size(0), context.size(1)
    ...
    caps_entity_idx_i = caps_entity_idx_i.unsqueeze(2).repeat(1,1,sourceL)#(n_context, queryL, sourceL)
    image_entity_idxs = image_entity_idxs.unsqueeze(1).repeat(1,queryL,1)#(n_context, queryL, sourceL)
        
    attn = torch.where(caps_entity_idx_i==image_entity_idxs,torch.full_like(caps_entity_idx_i, 1.0/queryL), torch.full_like(caps_entity_idx_i, 0))
    #attn = torch.where(caps_entity_idx_i==image_entity_idxs,torch.full_like(caps_entity_idx_i, 1), torch.full_like(caps_entity_idx_i, 0))
    attn = attn.view(batch_size, queryL, sourceL)#(n_context, queryL, sourceL)
        
    attnT = torch.transpose(attn, 1, 2).contiguous()#(n_context, sourceL, queryL)
    contextT = torch.transpose(context, 1, 2)#(n_context, sourceL, d)->(n_context, d, sourceL)
    weightedContext = torch.bmm(contextT.float().cuda(), attnT.float().cuda())#(n_context, d, sourceL)*(n_context, sourceL, queryL)->(n_context, d, queryL)
    weightedContext = torch.transpose(weightedContext, 1, 2)#(n_context, queryL, d)
...
def direct_attention_score(im, s, image_entity_idxs, caps_entity_idxs, cap_lens, opt):
    ...
    for i in range(batch_size_txt):
        ...
        caps_entity_idx_i = caps_entity_idxs[i, :n_word].unsqueeze(0)
        caps_entity_idx_i_expand = caps_entity_idx_i.repeat(batch_size_im, 1)
        txt_one = s[i, :n_word, :].unsqueeze(0) #(img_part_num,embed_size)       
        txt_one_expand = txt_one.repeat(batch_size_im, 1, 1)#(img_part_num,embed_size)->(batch_size_im,img_part_num,embed_size)
        #(batch_size_im,img_part_num,embed_size) cos (batch_size_im,img_part_num,embed_size)
        weiContext = func_direct_attention(txt_one_expand, im, caps_entity_idx_i_expand, image_entity_idxs, opt)
        score_i_all = cosine_similarity(txt_one_expand, weiContext, dim=2).view(batch_size_im,n_word)
        
        ####Average####
        score_i_avg = score_i_all.mean(dim=1, keepdim=False)
    ...
    #        caps_entity_idx_i = caps_entity_idxs[i].unsqueeze(1).repeat(1,img_part_num)
    #        print('caps_entity_idx_i',caps_entity_idx_i.shape)
    #        image_entity_idx_j = image_entity_idxs[j].unsqueeze(0).repeat(n_word,1)
    #        print('image_entity_idx_j',image_entity_idx_j.shape)
    #        txt_one = s[i,:n_word]
    #        print('txt_one',txt_one.shape)
    #        im_one = im[j]
    #        print('im_one',im_one.shape)
    #        mask_i_j = (caps_entity_idx_i==image_entity_idx_j)
    #        print('mask_i_j',mask_i_j)
    #        score_i_j = cosine_similarity(txt_one, im_one, dim=1)
    #        print('score_i_j',score_i_j.shape)
    #        I = Variable(mask_i_j)
    #        if torch.cuda.is_available():
    #           I = I.cuda()
    #        score_i_j = score_i_j.masked_fill_(I, 0)
    #        #print(score_i_j)
