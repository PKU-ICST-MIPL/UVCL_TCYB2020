...
import torch
from model import SCAN, joint_embedding_loss
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    sio.savemat('flickr_whole.mat',{'test_img':img_embs,'test_txt':cap_embs})
    print('Saving success...')
    
    #start = time.time()
    #sims = compute_sims(img_embs, cap_embs, cap_lens, img_att_w, opt, shard_size=128)
    #end = time.time()
...
def compute_sims(images, captions, caplens, opt, shard_size=128):
    """
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            sim = joint_embedding_loss(im, s, opt)
